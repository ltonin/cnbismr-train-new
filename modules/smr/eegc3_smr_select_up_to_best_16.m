function selection = eegc3_smr_select_up_to_best_16(cdataset, settings)
%
% function selection = eegc3_smr_select_stable(cdataset, settings)
% 
%
% Function to perform feature selection (CVA or dptools), either
% identifying stable features across runs or batch (using the whole
% dataset). It outputs up to the best 16 features.
%
% Inputs:
%
...
selection.Alltot = 0;
%% taking up to the best 16 features
if length(cidx) < 16 % where 16 is the number of maximum discriminant features we want to take!
    for i = 1:length(cidx)
        ch = cidx(i);
        bn = bidx(i);
