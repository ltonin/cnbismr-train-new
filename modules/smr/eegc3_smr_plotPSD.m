...
trial_length = bci.dur(4); % Continuous control event
...
for class = 1:length(bci.MI.task)
    % Average PSDs
    tmp_idx = find(bci.lbl_sample ==  bci.MI.task(class));
    tmp_trials_number = length(tmp_idx)/trial_dur;
    
    
    Max = max(max(bci.afeats_));
    Min = min(min(bci.afeats));
